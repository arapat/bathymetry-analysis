...
def run_test(features_test, labels_test, pkl_model_path, model_region, region):
    ...
    logger("eval, {}, {}, {}, {}, {}, {}, {}".format(model_region, region, model.num_trees(), loss, auprc, auroc, acc))
    return (true, scores)
...
def main_test(config, is_read_text):
    ...
    for model_region in regions:
        for region in regions:
            ...
            pkl_model_path = os.path.join(base_dir, '{}_model.pkl'.format(model_region))
            (true, scores) = run_test(features_test, labels_test, pkl_model_path, model_region, region)
            # with open("testing_result_{}_{}.pkl".format(model_region, region), 'wb') as fout:
